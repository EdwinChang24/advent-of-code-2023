...
fun part2() {
    ...
    fun List<Char>.tilted() =
        indices.filter { this[it] == 'O' }.map { index -> slice(0..<index).indexOfLast { it == '#' } }.groupBy { it }
            .mapValues { (_, list) -> list.size }.map { (index, count) -> (index + 1)..(index + count) }.let { ranges ->
    ...
    fun List<List<Char>>.spinCycle() =
        map { it.toList() }.transposed().map { it.tilted() }.transposed().map { it.tilted() }.reversed().transposed()
    ...
    println((1..1_000_000_000).first { i ->
        ...
        (1..i).runningFold(input.map { it.toList() }) { acc, _ -> acc.spinCycle() }.map { it.toString() }.let { list ->
